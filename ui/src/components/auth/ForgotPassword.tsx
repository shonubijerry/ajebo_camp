...
export default function ForgotPassword({
  ...
  const handleChangeSubmit = async (data: ChangePayload) => {
    try {
      ...
      if (data.password !== data.confirmPassword) {
        changeForm.setError('confirmPassword', {
          type: 'validate',
  ...
  const isLoading =
    step === 'request' ? requestMutation.isPending : changeMutation.isPending
  ...
  return (
    ...
    >
      <DialogTitle>
        {step === 'request' ? 'Reset password' : 'Change password'}
      </DialogTitle>
      <DialogContent
